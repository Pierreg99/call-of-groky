...
export function buildArena(scene: THREE.Scene, shadowMapSize = 2048): ArenaBuild {
  ...
  // Loop 6 — Control tower landmark (dense composition for screenshots)
  const towerX = 12;
  const towerZ = -18;
  const towerCenter = new THREE.Vector3(towerX, 0, towerZ);

  // Tower base / walls (open south toward courtyard)
  const twWalls: Array<[number, number, number, number, number, number, THREE.Material]> = [
    [8.5, 5.5, 0.55, towerX, 2.75, towerZ - 3.6, dark], // north
    [0.55, 5.5, 7.5, towerX - 4.1, 2.75, towerZ, dark], // west
    [0.55, 5.5, 7.5, towerX + 4.1, 2.75, towerZ, dark], // east
    [2.8, 5.5, 0.55, towerX - 2.6, 2.75, towerZ + 3.6, dark], // south L
    [2.8, 5.5, 0.55, towerX + 2.6, 2.75, towerZ + 3.6, dark], // south R
  ];
  ...
  // Exterior façade readables (screenshot identity)
  ...
  // Interior stairs (segmented pads)
  for (let i = 0; i < 7; i++) {
    const t = i / 6;
  ...
  // Control consoles / monitors (composition props)
  ...
  // Window frames on north wall (readable silhouette)
  ...
  // Warehouse aisle density inside tower ground floor
  ...
  const ammoSpots: Array<[number, number, number]> = [
    [0.5, 0, 1.5],
    [towerX - 1.0, 0, towerZ + 1.0],
    [-14, 0, -2],
    [8, 0, 11],
    [towerX + 0.5, 3.55, towerZ + 0.5],
  ];
